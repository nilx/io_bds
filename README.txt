% io_bin: simplest image io interface
...
io_bin.c contains simple routines to read and write binary image data.
Is has the lowest possible overhead, but is not portable across
...
The latst version of this code is available at
  http://dev.ipol.im/git/?p=nil/io_bin.git
with a mirror at
  https://github.com/nilx/io_bin

# REQUIREMENTS

io_bin.c is ANSI C, and should compile on any system with any ANSI C
compiler.

# COMPILATION

Compile io_bin.c with the same compiler used for the other source files
of your project. C and C++ compilers are possible. io_bin has been
...
Compile io_bin.c with your program, and include io_bin.h to get the
function declarations. You can use io_bin.c with C or C++ code.
...
* io_bin_read_flt(fname, &nx, &ny, &nc)
  read a float array
